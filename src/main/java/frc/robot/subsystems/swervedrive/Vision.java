...
public class Vision {
  ...
  private void openSimCameraViews() {
    if (Desktop.isDesktopSupported() && Desktop.getDesktop().isSupported(Desktop.Action.BROWSE)) {
      ...
      // / URI("http://localhost:1186/")); / } catch (IOException | URISyntaxException
      // e) / { /
      // / e.printStackTrace(); / }
    }
  }
  ...
  /**
   * Find the AprilTag that occupies the most pixels in the current camera frame.
   *
   * @param result a non-null, *single* PhotonPipelineResult (you already check
   *               result.hasTargets()).
   * @return OptionalInt containing the fiducial ID, or empty if no tag found.
   */
  public static OptionalInt getLargestTagId(PhotonPipelineResult result) {
    return result.getTargets() // all tracked targets in this frame :contentReference[oaicite:0]{index=0}
        .stream()
        .max(Comparator.comparingDouble(PhotonTrackedTarget::getArea)) // pick biggest by %-image area
                                                                       // :contentReference[oaicite:1]{index=1}
        .map(PhotonTrackedTarget::getFiducialId) // extract its AprilTag ID :contentReference[oaicite:2]{index=2}
        .stream()
  ...
  enum Cameras {
    ...
    private void updateUnreadResults() {
      ...
      boolean passThroughPhotonPipeline = false;
      for (PhotonPipelineResult result : resultsList) {
      ...
      //     (currentTimestamp - lastReadTimestamp) >= debounceTime) {
        ...
        if (!resultsList.isEmpty()) {
          ...
          passThroughPhotonPipeline = true;
      //  } 
      }
      SmartDashboard.putBoolean("resultsNotEmpty", passThroughPhotonPipeline);
